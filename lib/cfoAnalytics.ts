...
export function calculateCFOAnalytics(data: CFODataPackage) {
  ...
  const wasteMovements = inventory_movements.filter(m => 
    m.type === 'out' && (m.reason.toLowerCase().includes('waste') || m.reason.toLowerCase().includes('spoil') || m.reason.toLowerCase().includes('loss') || m.reason.toLowerCase().includes('damage'))
  );
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const dayOrders = completedOrders.filter(o => o.createdAt.startsWith(dStr));
    const sales = dayOrders.reduce((sum, o) => sum + (Number(o.totalAmount) || 0), 0) || Math.round(avgDailySales * (0.85 + Math.random() * 0.3));
    const exp = Math.round(sales * 0.65);
    const prof = sales - exp;
