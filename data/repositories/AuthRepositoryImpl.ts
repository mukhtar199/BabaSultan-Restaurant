...
export class AuthRepositoryImpl implements IAuthRepository {
  async getCurrentUser(): Promise<UserProfile | null> {
    ...
    if (!firebaseUser) {
      // Check localStorage for active session or demo role
  ...
  async loginWithGoogle(): Promise<UserProfile> {
    ...
    if (snap.exists()) {
      ...
      profile = {
        uid: u.uid,
        email: u.email || 'admin@restaurant.com',
        displayName: u.displayName || 'Restaurant Executive',
        role: u.email === 'adspace119@gmail.com' ? 'admin' : 'cashier',
        createdAt: new Date().toISOString()
      };
