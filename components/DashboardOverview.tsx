...
export const DashboardOverview: React.FC<DashboardOverviewProps> = ({
  ...
  const todayCogs = todayOrders.reduce((acc, o) => acc + o.cogs, 0);
  ...
  const totalOverdueAmount = overduePurchases.reduce((acc, p) => acc + p.totalCost, 0);
      ...
      [
        {
          ...
          rows: [
            ["Today Sales Revenue", `$${todayRevenue.toFixed(2)}`, "Completed"],
            ["Cost of Goods Sold (COGS)", `$${todayCogs.toFixed(2)}`, "Deducted"],
            ["Today Operational Expenses", `$${todayExpensesTotal.toFixed(2)}`, "Deducted"],
            ["Net Profit Today", `$${todayNetProfit.toFixed(2)}`, todayNetProfit >= 0 ? "Profitable" : "Deficit"]
          ]
        },
        {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-amber-200/80">
                ...
                {overduePurchases.length > 0 && `${overduePurchases.length} supplier invoices overdue ($${totalOverdueAmount.toFixed(2)}).`}
              </p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-5 rounded-2xl bg-slate-900 border border-slate-800 shadow-sm relative overflow-hidden">
          ...
          <p className="text-[11px] text-slate-500 mt-1 flex items-center gap-1">
            <span className="text-emerald-400 font-semibold">+Revenue: ${todayRevenue.toFixed(2)}</span>
          </p>
        </div>
