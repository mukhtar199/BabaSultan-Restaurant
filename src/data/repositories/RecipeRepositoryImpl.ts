...
export class RecipeRepositoryImpl implements IRecipeRepository {
  ...
  async createIngredient(
    ...
    if (ingredientData.currentStockUsageUnit > 0) {
      await addDoc(collection(db, INGREDIENT_MOVEMENTS_COLL), {
        ingredientId: ref.id,
        ingredientName: newIng.name,
        type: 'purchase',
        quantity: newIng.currentStockUsageUnit,
        unit: newIng.usageUnit,
        previousStock: 0,
        newStock: newIng.currentStockUsageUnit,
        cost: newIng.purchaseCost,
      ...
      for (const orderItem of orderItems) {
        ...
        for (const recipeItem of recipe.items) {
          ...
          await addDoc(collection(db, INGREDIENT_MOVEMENTS_COLL), {
            ...
            unit: ingData.usageUnit,
            previousStock,
            newStock,
            cost: deductedInUsageUnit * (ingData.costPerUsageUnit || 0),
      ...
      if (item.difference !== 0) {
        ...
        if (ingSnap.exists()) {
          ...
          await addDoc(collection(db, INGREDIENT_MOVEMENTS_COLL), {
            ...
            previousStock,
            newStock,
            cost: Math.abs(item.difference) * item.costPerUnit,
            referenceId: countData.countNumber,
  ...
  async recordWaste(data: Omit<WasteRecord, 'id' | 'createdAt'>): Promise<WasteRecord> {
    ...
    if (ingSnap.exists()) {
      ...
      await addDoc(collection(db, INGREDIENT_MOVEMENTS_COLL), {
        ...
        cost: data.totalCost,
        referenceId: ref.id,
        reason: `Waste Recorded: ${data.reason.replace('_', ' ').toUpperCase()} - ${data.notes || ''}`,
