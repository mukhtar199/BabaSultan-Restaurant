    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl space-y-1">
          ...
          <span className="text-xl font-black text-blue-400">{analytics.retentionRate.toFixed(1)}%</span>
        </div>

        <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl space-y-1">
          <span className="text-[10px] font-bold text-slate-500 uppercase block">Churn Rate</span>
          <span className="text-xl font-black text-rose-400">{analytics.churnRate.toFixed(1)}%</span>
        </div>

        <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl space-y-1">
          <span className="text-[10px] font-bold text-slate-500 uppercase block">Wallet Balance</span>
          <span className="text-xl font-black text-amber-400">${analytics.totalWalletBalance.toFixed(2)}</span>
        </div>

        <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl space-y-1">
          <span className="text-[10px] font-bold text-slate-500 uppercase block">Avg CLV</span>
          <span className="text-xl font-black text-purple-400">${analytics.avgCustomerLifetimeValue.toFixed(2)}</span>
        </div>
      </div>
