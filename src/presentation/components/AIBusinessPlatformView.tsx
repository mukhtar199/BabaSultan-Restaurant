...
export const AIBusinessPlatformView: React.FC<AIBusinessPlatformViewProps> = ({
  ...
  const handleExportSummaryExcel = () => {
    ...
    const rows = [
      ...
      ['AI Accountant', 'Today Revenue', `$${analytics.todayRevenue.toFixed(2)}`, 'Sales Inflow'],
      ['AI Accountant', 'Today Net Profit', `$${analytics.todayNetProfit.toFixed(2)}`, 'Bottom-line Profit'],
      ['AI Operations', 'Avg Kitchen Prep Time', `${analytics.avgPrepTimeMinutes} min`, 'Optimal Target < 15 min'],
      ...
      ['AI Forecasting', 'Projected 30-Day Sales', `$${analytics.projected30DaySales.toLocaleString()}`, 'Growth Trend +8%']
    ];
      ...
      <div className="bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 border border-slate-800 rounded-3xl p-6 sm:p-8 text-white shadow-2xl relative overflow-hidden">
        ...
        <div className="mt-6 pt-6 border-t border-slate-800 grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3 relative z-10">
          ...
          <div className="bg-slate-900/80 p-3.5 rounded-2xl border border-slate-800">
            ...
            <span className="text-sm font-extrabold text-emerald-400 mt-0.5 block">${analytics.todayRevenue.toFixed(2)}</span>
          </div>

          <div className="bg-slate-900/80 p-3.5 rounded-2xl border border-slate-800">
            <span className="text-[10px] text-slate-400 uppercase font-bold block">Today Net Profit</span>
            <span className={`text-sm font-extrabold mt-0.5 block ${analytics.todayNetProfit >= 0 ? 'text-indigo-400' : 'text-rose-400'}`}>
              ${analytics.todayNetProfit.toFixed(2)}
            </span>
          </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 space-y-4">
              ...
              <div className="space-y-3 text-xs">
                <div className="flex justify-between items-center p-3 rounded-2xl bg-slate-950 border border-slate-800">
                  ...
                  <span className="font-extrabold text-emerald-400">${analytics.projected30DaySales.toLocaleString()}</span>
                </div>
                <div className="flex justify-between items-center p-3 rounded-2xl bg-slate-950 border border-slate-800">
                  <span className="text-slate-400">Projected Monthly Profit</span>
                  <span className="font-extrabold text-indigo-400">${analytics.projectedMonthlyProfit.toLocaleString()}</span>
                </div>
                <div className="flex justify-between items-center p-3 rounded-2xl bg-slate-950 border border-slate-800">
                  <span className="text-slate-400">Projected Operating Expenses</span>
                  <span className="font-extrabold text-slate-300">${analytics.projectedMonthlyExpenses.toLocaleString()}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 space-y-6">
                  ...
                  printReportWindow('AI Certified Financial Audit Statement', 'Real-time Firestore Ledger', [
                    {
                      ...
                      rows: [
                        ['Gross Revenue', `$${analytics.totalRevenue.toFixed(2)}`, 'Audited'],
                        ['Total Expenses', `$${analytics.totalExpenses.toFixed(2)}`, 'Verified'],
                        ['Net Operating Profit', `$${analytics.netProfit.toFixed(2)}`, 'Healthy'],
                        ['Cash Balance Liquid Reserve', `$${analytics.liquidBalance.toFixed(2)}`, 'Sufficient']
                      ]
                    }
                  ]);
                }}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
              <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-1">
                ...
                <span className="text-lg font-bold text-emerald-400 block">${analytics.totalRevenue.toFixed(2)}</span>
              </div>
              <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-1">
                <span className="text-xs text-slate-400 font-medium block">Total Operating Expenses</span>
                <span className="text-lg font-bold text-rose-400 block">${analytics.totalExpenses.toFixed(2)}</span>
              </div>
              <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-1">
                <span className="text-xs text-slate-400 font-medium block">Net Operating Profit</span>
                <span className="text-lg font-bold text-indigo-400 block">${analytics.netProfit.toFixed(2)}</span>
              </div>
              <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-1">
                <span className="text-xs text-slate-400 font-medium block">Liquid Reserve Balance</span>
                <span className="text-lg font-bold text-teal-400 block">${analytics.liquidBalance.toFixed(2)}</span>
              </div>
            </div>
              ...
              <div className="space-y-3">
                {analytics.bestSellingProducts.map((p, idx) => (
                  <div key={idx} className="p-3.5 rounded-2xl bg-slate-950 border border-slate-800 flex items-center justify-between text-xs">
                    ...
                    <span className="font-extrabold text-emerald-400">${p.revenue.toFixed(2)} Revenue</span>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-3">
                ...
                {analytics.purchasingRecommendations.map((pr, idx) => (
                  <div key={idx} className="flex justify-between items-center text-xs p-2.5 rounded-xl bg-slate-900 border border-slate-800">
                    ...
                    <span className="font-extrabold text-emerald-400">Est. Order: ${pr.suggestedOrderValuation.toFixed(2)}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-1">
                ...
                <span className="text-xl font-bold text-emerald-400 block">${analytics.projectedNextDaySales.toLocaleString()}</span>
              </div>

              <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-1">
                <span className="text-xs text-slate-400 font-medium block">7-Day Revenue Forecast</span>
                <span className="text-xl font-bold text-indigo-400 block">${analytics.projected7DaySales.toLocaleString()}</span>
              </div>

              <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-1">
                <span className="text-xs text-slate-400 font-medium block">30-Day Revenue Forecast</span>
                <span className="text-xl font-bold text-purple-400 block">${analytics.projected30DaySales.toLocaleString()}</span>
              </div>
            </div>
          </div>
        </div>
