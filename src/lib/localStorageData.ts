/**
 * DATA ARCHITECTURE POLICY & STORAGE CONTRACT
 * =================================================================
 * 1. AUTHORITATIVE DATA STORE: Cloud Firestore is the absolute single source of truth for
 *    all persistent ERP operational, transactional, and financial records.
 * 2. LOCAL STORAGE SCOPE: LocalStorage is strictly reserved for:
 *    - Setup Wizard temporary draft state before remote Firestore submission
 *    - Non-authoritative client UI preferences (e.g. active tab, draft forms)
 *    - Fallback client-side cache during offline initial onboarding
 * 3. SYNC CONTRACT:
 *    - Writes: Setup Wizard form submissions persist remotely to Firestore via atomic batches.
 *      LocalStorage holds secondary draft/cache.
 *    - Reads: Active app UI subscribes to real-time Firestore collection listeners (onSnapshot).
 *      LocalStorage state is read only as fallback if Firestore collections are empty or offline.
 * =================================================================
 */
...
export function parseSetupWizardData(setupData: InitialSetupData): LocalStorageState {
  ...
  const users = employees.map(emp => ({
    uid: `user_${emp.id}`,
    email: emp.email || `${(emp.name || 'user').toLowerCase().replace(/\s+/g, '.')}@restaurant.com`,
