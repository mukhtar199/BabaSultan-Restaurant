...
export function calculateAIBusinessPlatformAnalytics(data: AIPlatformDataPackage) {
  ...
  const totalCOGS = completedOrders.reduce((sum, o) => sum + (o.cogs || o.totalAmount * 0.45), 0);
  const todayCOGS = todayCompleted.reduce((sum, o) => sum + (o.cogs || o.totalAmount * 0.45), 0);
