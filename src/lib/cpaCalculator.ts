...
export function calculateCPAMetrics(data: CPADataInput): CPAMetrics {
  ...
  const cashBalance = cashAcc ? cashAcc.balance : 1850.00;
  const bankBalance = bankAcc ? bankAcc.balance : 12450.00;
  ...
  // Taxes (5% Estimated VAT, 15% Corporate Tax on net profit)
  const taxEstimatedVAT = netRevenue * 0.05;
