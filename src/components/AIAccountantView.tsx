...
export const AIAccountantView: React.FC<AIAccountantViewProps> = ({
  ...
  const allLedgerEntries = [
    ...orders.filter(o => o.status === 'completed').map(o => ({
      id: `ord_${o.id}`,
      type: 'income',
      title: `Customer Order ${o.orderNumber} (${o.customerName})`,
    ...
    ...purchases.map(p => ({
      id: `pur_${p.id}`,
      type: 'purchase',
      title: `Supplier Purchase: ${p.itemName} (${p.supplierName})`,
      category: 'Inventory AP',
      amount: -p.totalCost,
      date: p.createdAt,
      method: p.status.toUpperCase()
    })),
    ...salaries.map(s => ({
      id: `sal_${s.id}`,
      type: 'payroll',
      title: `Payroll Salary: ${s.employeeName}`,
      category: `Salary (${s.period})`,
      amount: -s.amount,
      date: s.paidDate,
      method: s.status.toUpperCase()
    })),
    ...refunds.map(r => ({
      id: `ref_${r.id}`,
      type: 'refund',
      title: `Customer Refund: ${r.customerName}`,
    ...
    ...bankTransactions.map(b => ({
      id: `bt_${b.id}`,
      type: 'bank',
      title: `Bank ${b.type.toUpperCase()}: ${b.description}`,
      category: b.accountName,
      amount: b.type === 'deposit' ? b.amount : -b.amount,
      date: b.createdAt,
      method: b.reference
    }))
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 text-white shadow-xl relative overflow-hidden">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3 mt-6 pt-6 border-t border-slate-800 text-slate-200">
          <div className="bg-slate-800/60 p-3 rounded-xl border border-slate-700/60">
            ...
            <p className="text-lg font-bold text-emerald-400 mt-1">${metrics.grossRevenue.toFixed(2)}</p>
          </div>
          <div className="bg-slate-800/60 p-3 rounded-xl border border-slate-700/60">
            <p className="text-[11px] text-slate-400 font-medium uppercase tracking-wider">Net Revenue</p>
            <p className="text-lg font-bold text-teal-300 mt-1">${metrics.netRevenue.toFixed(2)}</p>
          </div>
          <div className="bg-slate-800/60 p-3 rounded-xl border border-slate-700/60">
            <p className="text-[11px] text-slate-400 font-medium uppercase tracking-wider">Net Profit</p>
            <p className={`text-lg font-bold mt-1 ${metrics.netProfit >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
          ...
          <div className="bg-slate-800/60 p-3 rounded-xl border border-slate-700/60">
            ...
            <div className="flex items-center gap-1.5 mt-1">
              <span className="text-lg font-bold text-amber-300">{metrics.foodCostPercentage.toFixed(1)}%</span>
              <span className={`text-[10px] px-1.5 py-0.5 rounded ${metrics.foodCostPercentage <= 35 ? 'bg-emerald-500/20 text-emerald-300' : 'bg-rose-500/20 text-rose-300'}`}>
                {metrics.foodCostPercentage <= 35 ? 'Target' : 'High'}
              </span>
            </div>
          </div>
          <div className="bg-slate-800/60 p-3 rounded-xl border border-slate-700/60">
            <p className="text-[11px] text-slate-400 font-medium uppercase tracking-wider">Labor Cost %</p>
            <p className="text-lg font-bold text-sky-300 mt-1">{metrics.laborCostPercentage.toFixed(1)}%</p>
          </div>
          <div className="bg-slate-800/60 p-3 rounded-xl border border-slate-700/60">
            <p className="text-[11px] text-slate-400 font-medium uppercase tracking-wider">Cash Drawer</p>
            <p className="text-lg font-bold text-white mt-1">${metrics.cashBalance.toFixed(2)}</p>
          </div>
          <div className="bg-slate-800/60 p-3 rounded-xl border border-slate-700/60">
            <p className="text-[11px] text-slate-400 font-medium uppercase tracking-wider">Bank Balance</p>
            <p className="text-lg font-bold text-indigo-300 mt-1">${metrics.bankBalance.toFixed(2)}</p>
          </div>
          <div className="bg-slate-800/60 p-3 rounded-xl border border-slate-700/60">
            <p className="text-[11px] text-slate-400 font-medium uppercase tracking-wider">Accounts Payable</p>
            <p className="text-lg font-bold text-rose-400 mt-1">${metrics.accountsPayable.toFixed(2)}</p>
          </div>
        </div>
      </div>
            ...
            <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-4">
              ...
              <div className="space-y-3 pt-2">
                <div className="flex items-center justify-between text-sm py-2 border-b border-slate-100">
                  ...
                  <span className="font-bold text-slate-900">${metrics.dailySales.toFixed(2)}</span>
                </div>
                <div className="flex items-center justify-between text-sm py-2 border-b border-slate-100">
                  <span className="text-slate-600 font-medium">Weekly Sales</span>
                  <span className="font-bold text-slate-900">${metrics.weeklySales.toFixed(2)}</span>
                </div>
                <div className="flex items-center justify-between text-sm py-2 border-b border-slate-100">
                  <span className="text-slate-600 font-medium">Cost of Goods Sold (COGS)</span>
                  <span className="font-bold text-slate-900">${metrics.cogs.toFixed(2)}</span>
                </div>
                <div className="flex items-center justify-between text-sm py-2 border-b border-slate-100">
                  <span className="text-slate-600 font-medium">Total Inventory Asset Value</span>
                  <span className="font-bold text-slate-900">${metrics.inventoryValuation.toFixed(2)}</span>
                </div>
                <div className="flex items-center justify-between text-sm py-2 border-b border-slate-100">
                  <span className="text-slate-600 font-medium">Total Liquid Capital (Cash + Bank)</span>
                  <span className="font-bold text-emerald-600">${metrics.totalLiquidity.toFixed(2)}</span>
                </div>
              </div>
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-2">
                ...
                <div className="flex justify-between text-xs text-slate-600">
                  ...
                  <span className="font-bold text-emerald-700">${metrics.predictedMonthlyProfit.toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-xs text-slate-600">
                  <span>Predicted Monthly Expenses:</span>
                  <span className="font-bold text-slate-800">${metrics.predictedFutureExpenses.toFixed(2)}</span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4 text-slate-800 text-sm">
              ...
              <div className="border border-slate-200 rounded-xl overflow-hidden divide-y divide-slate-100">
                ...
                <div className="px-4 py-2.5 flex justify-between">
                  ...
                  <span className="font-semibold">${metrics.grossRevenue.toFixed(2)}</span>
                </div>
                <div className="px-4 py-2.5 flex justify-between text-rose-600">
                  <span>Less: Customer Refunds & Allowances</span>
                  <span className="font-semibold">-${metrics.customerRefundsTotal.toFixed(2)}</span>
                </div>
                <div className="px-4 py-3 flex justify-between bg-emerald-50 font-bold text-slate-900 border-t border-b border-emerald-200">
                  ...
                  <span className="text-emerald-700">${metrics.netRevenue.toFixed(2)}</span>
                </div>
                ...
                <div className="px-4 py-2.5 flex justify-between text-slate-700">
                  ...
                  <span className="font-semibold">-${metrics.cogs.toFixed(2)}</span>
                </div>
                <div className="px-4 py-3 flex justify-between bg-teal-50 font-bold text-slate-900 border-t border-b border-teal-200">
                  <span>GROSS PROFIT</span>
                  <span className="text-teal-800">${metrics.grossProfit.toFixed(2)}</span>
                </div>
                ...
                <div className="px-4 py-2.5 flex justify-between">
                  ...
                  <span className="font-semibold">-${metrics.laborCost.toFixed(2)}</span>
                </div>
                <div className="px-4 py-2.5 flex justify-between">
                  <span>Utilities & Generator Diesel Fuel</span>
                  <span className="font-semibold">-${metrics.operatingExpenses.toFixed(2)}</span>
                </div>
                <div className="px-4 py-2.5 flex justify-between">
                  <span>Delivery Logistics</span>
                  <span className="font-semibold">-${metrics.deliveryCost.toFixed(2)}</span>
                </div>
                <div className="px-4 py-3 flex justify-between bg-slate-100 font-bold text-slate-900 border-t border-b border-slate-200">
                  ...
                  <span className="text-slate-900">-${metrics.totalExpenses.toFixed(2)}</span>
                </div>

                <div className="p-4 bg-slate-900 text-white rounded-b-xl flex justify-between items-center text-base font-bold">
                  <span>NET OPERATING PROFIT</span>
                  <span className={metrics.netProfit >= 0 ? 'text-emerald-400 text-xl' : 'text-rose-400 text-xl'}>
                    ${metrics.netProfit.toFixed(2)}
                  </span>
                </div>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-slate-50 p-5 rounded-2xl border border-slate-200 space-y-3">
                  ...
                  <p className="text-2xl font-black text-slate-900">${metrics.cashBalance.toFixed(2)}</p>
                </div>
                ...
                <div className="bg-slate-50 p-5 rounded-2xl border border-slate-200 space-y-3">
                  ...
                  <p className="text-2xl font-black text-slate-900">${metrics.bankBalance.toFixed(2)}</p>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-5 rounded-2xl border border-slate-200 bg-emerald-50/50 space-y-2">
                  ...
                  <p className="text-3xl font-black text-emerald-800">${metrics.taxEstimatedVAT.toFixed(2)}</p>
                  <p className="text-xs text-slate-500">Calculated on Net Sales Revenue (${metrics.netRevenue.toFixed(2)})</p>
                </div>
                ...
                <div className="p-5 rounded-2xl border border-slate-200 bg-sky-50/50 space-y-2">
                  ...
                  <p className="text-3xl font-black text-sky-800">${metrics.taxEstimatedCorporate.toFixed(2)}</p>
                  <p className="text-xs text-slate-500">Calculated on Net Operating Profit (${metrics.netProfit.toFixed(2)})</p>
                </div>
              </div>
            </div>
          )}
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {filteredLedger.map((entry) => (
                  <tr key={entry.id} className="hover:bg-slate-50/80 transition">
                    ...
                    <td className={`p-3 text-right font-bold text-sm ${entry.amount > 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
