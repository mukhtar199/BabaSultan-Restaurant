              ...
              <tbody className="divide-y divide-slate-800/60">
                {sortedStaff.map((emp, rank) => (
                  <tr key={emp.id} className="hover:bg-slate-800/40 transition">
                    ...
                    <td className="py-4 px-6 text-slate-200">${(emp.salary || 0).toFixed(2)}</td>
                    <td className="py-4 px-6 font-bold text-emerald-400">${(emp.totalSales || 0).toFixed(2)}</td>
                    <td className="py-4 px-6 text-slate-300">{emp.ordersCount || 0} orders</td>
              ...
              <tbody className="divide-y divide-slate-800/60">
                {suppliers.map(sup => (
                  <tr key={sup.id} className="hover:bg-slate-800/40 transition">
                    ...
                    <td className="py-4 px-6 font-bold text-slate-200">${(sup.pendingAmount || 0).toFixed(2)}</td>
                    <td className="py-4 px-6 font-bold">
                      {(sup.overdueAmount || 0) > 0 ? (
                        <span className="text-rose-400 flex items-center gap-1">
                          <AlertTriangle className="w-3.5 h-3.5" />
                          ${(sup.overdueAmount || 0).toFixed(2)} OVERDUE
                        </span>
                      ) : (
