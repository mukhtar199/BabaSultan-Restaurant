...
export const FinancialsView: React.FC<FinancialsViewProps> = ({
  ...
  const [salaryPeriod, setSalaryPeriod] = useState(() => new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' }));
              ...
              <tbody className="divide-y divide-slate-800/60">
                {salaries.map(s => (
                  <tr key={s.id} className="hover:bg-slate-800/40 transition">
                    ...
                    <td className="py-4 px-6 font-bold text-emerald-400">${(s.amount || 0).toFixed(2)}</td>
                    <td className="py-4 px-6 text-xs text-slate-300">{s.period}</td>
                    <td className="py-4 px-6 text-xs font-mono font-bold text-emerald-400">{(s.status || 'PAID').toUpperCase()}</td>
