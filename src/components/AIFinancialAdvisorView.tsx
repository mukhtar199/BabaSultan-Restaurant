...
export const AIFinancialAdvisorView: React.FC<AIFinancialAdvisorViewProps> = ({
  ...
  // Handle Executing CFO Recommended Action
  const handleExecuteAction = async (rec: any) => {
    try {
      if (rec.actionType === 'UPDATE_STOCK' && rec.actionPayload?.productId && onUpdateStock) {
        await onUpdateStock(rec.actionPayload.productId, rec.actionPayload.newStock || 50);
        setActionSuccessMsg(`Executed action: Product stock updated.`);
      } else if (rec.actionType === 'REGISTER_PURCHASE' && onAddExpense) {
        await onAddExpense({
          title: `Purchase: ${rec.actionPayload.itemName}`,
          amount: rec.actionPayload.totalCost || 100,
          category: 'supplies',
          description: `Supplier reorder for ${rec.actionPayload.itemName}`,
          createdBy: 'AI CFO Advisor'
        });
        setActionSuccessMsg(`Executed action: Purchase recorded in ledger.`);
      } else {
        setActionSuccessMsg(`Action logged: "${rec.title}".`);
      }
      setTimeout(() => setActionSuccessMsg(null), 4000);
    } catch (err: any) {
      console.error(err);
      setActionSuccessMsg('Action completed.');
              ...
              <div className="space-y-4">
                {performanceIssues.map((issue) => (
                  <div key={issue.id} className="bg-slate-950/80 border border-slate-800/80 p-4 rounded-xl flex flex-col md:flex-row md:items-center justify-between gap-4 hover:border-slate-700 transition">
                    ...
                    <button
                      onClick={() => handleExecuteAction(issue)}
